Public Class FrmEstructuraPresup
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(FrmEstructuraPresup))
        ...
        Me.Name = "FrmEstructuraPresup"
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Estructura del Presupuesto"
...
#End Region
    ...
    Private mintIDPresup As Integer
    Private mstrNumPresup, mstrRevPresup, mstrDescPresup As String
    Private mdtEstructura As DataTable

#Region " Load "

    Public Sub AbrirEstructuraPresupuesto(ByVal intIDPresup As Integer, ByVal strNumPresup As String, _
                                          ByVal strRevPresup As String, ByVal strDescPresup As String)
        mintIDPresup = intIDPresup
        mstrNumPresup = strNumPresup
        mstrRevPresup = strRevPresup
        mstrDescPresup = strDescPresup

        Me.ShowDialog()
    End Sub

    Private Sub FrmEstructuraPresup_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        rbtMaximizarTree.Checked = True
        mdtEstructura = New BE.DataEngine().Filter("vFrmObraPresupTrabajos", New NumberFilterItem("IDPresup", mintIDPresup), , "Secuencia")
        VerDatos()
    End Sub
...
#End Region
    ...
    Private Sub VerDatos()
        If mintIDPresup > 0 And chkVerTrabajos.Checked Then
            TvwEstructura.Nodes.Clear()
            TvwEstructura.ImageList = ImageList1
            Dim strText As String = mstrNumPresup & " / " & mstrRevPresup
            If length(mstrDescPresup) > 0 Then strText = strText & " -> " & mstrDescPresup
...
#Region " Trabajos "
    ...
    Private Sub PintarTrabajos(ByVal dtTrabajos As DataTable, Optional ByVal oNode As TreeNode = Nothing)
        If Not dtTrabajos Is Nothing AndAlso dtTrabajos.Rows.Count > 0 Then
            ...
            For Each drTrabajo As DataRow In dtTrabajos.Select
                If IsDBNull(drTrabajo("IDTrabajoPresupPadre")) Then
    ...
    Private Sub PintarNodoTrabajo(ByVal drTrabajo As DataRow, ByVal dtTrabajos As DataTable, _
        ...
        If chkVerMateriales.Checked Then
            Dim dtMateriales As DataTable = New ObraPresupMaterial().Filter(f)
            PintarMateriales(dtMateriales, oNode)
        End If
        'pintar mod
        If chkVerMOD.Checked Then
            Dim dtMod As DataTable = New ObraPresupMOD().Filter(f)
            PintarMOD(dtMod, oNode)
        End If
        'pintar centros
        If chkVerCentros.Checked Then
            Dim dtC As DataTable = New ObraPresupCentro().Filter(f)
            PintarCentros(dtC, oNode)
        End If
        'pintar gastos
        If chkVerGastos.Checked Then
            Dim dtG As DataTable = New ObraPresupGasto().Filter(f)
            PintarGastos(dtG, oNode)
        End If
        'pintar varios
        If chkVerVarios.Checked Then
            Dim dtV As DataTable = New ObraPresupVarios().Filter(f)
            PintarVarios(dtV, oNode)
        End If
        'pintar mediciones
        If chkVerMediciones.Checked Then
            Dim dtME As DataTable = New ObraPresupMedicion().Filter(f)
        ...
        If Not dtMateriales Is Nothing AndAlso dtMateriales.Rows.Count > 0 Then
            ...
            For Each drMaterial As DataRow In dtMateriales.Select
                PintarNodoMaterial(drMaterial("IDMaterial") & String.Empty, _
                                   drMaterial("DescMaterial") & String.Empty, drMaterial("QPresup"), oNode)
            Next
        End If
        ...
        If Not dtMOD Is Nothing AndAlso dtMOD.Rows.Count > 0 Then
            ...
            For Each drMOD As DataRow In dtMOD.Select
                PintarNodoMOD(drMOD("IDCategoria") & String.Empty, drMOD("DescCategoria") & String.Empty, _
                              drMOD("DescHora") & String.Empty,  oNode)
            Next
            oNode = Nothing
        End If
    ...
    Private Sub PintarCentros(ByVal dtCentros As DataTable, Optional ByVal oNode As TreeNode = Nothing)
        If Not dtCentros Is Nothing AndAlso dtCentros.Rows.Count > 0 Then
            ...
            For Each drCentros As DataRow In dtCentros.Select
                PintarNodoCentros(drCentros("IDCentro") & String.Empty, drCentros("DescCentro") & String.Empty, oNode)
            Next
            oNode = Nothing
        End If
    End Sub
    ...
    Private Sub PintarGastos(ByVal dtGastos As DataTable, Optional ByVal oNode As TreeNode = Nothing)
        If Not dtGastos Is Nothing AndAlso dtGastos.Rows.Count > 0 Then
            ...
            For Each drGastos As DataRow In dtGastos.Select
                PintarNodoGastos(drGastos("IDGasto") & String.Empty, drGastos("DescGasto") & String.Empty,  oNode)
            Next
            oNode = Nothing
        End If
    End Sub
    ...
    Private Sub PintarVarios(ByVal dtVarios As DataTable, _
        ...
        If Not dtVarios Is Nothing AndAlso dtVarios.Rows.Count > 0 Then
            ...
            For Each drVarios As DataRow In dtVarios.Select
                PintarNodoVarios(drVarios("IDVarios") & String.Empty, drVarios("DescVarios") & String.Empty, oNode)
            Next
            oNode = Nothing
        End If
    End Sub
...
#Region " Mediciones "
    ...
    Private Sub PintarNodoMediciones(ByVal strDescMedicion As String, _
                                     ByVal dblQPI As Double, ByVal dblLargo As Double, _
                                     ByVal dblAncho As Double, ByVal dblAlto As Double, _
                                     ByVal dblTotal As Double, Optional ByVal oNode As TreeNode = Nothing)
